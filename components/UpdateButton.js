...
// V01 - 初始版本，手動更新股價按鈕 + 輪詢進度彈窗
// V02 - 「14:30前確認」彈窗支援 ESC 鍵關閉
//       （更新中/成功/失敗提示層不加 ESC，避免使用者誤關）
// =============================================================================
...
export default function UpdateButton({ onRefresh }) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => { if (e.key === "Escape") setShowConfirm(false); };
  ...
  const handleClickUpdate = () => {
    if (isBeforeMarketClose()) { setShowConfirm(true); }
    else { triggerUpdate(); }
  };
  ...
  const getLatestRunId = async () => {
    try {
      ...
      if (data.workflow_runs && data.workflow_runs.length > 0) return data.workflow_runs[0].id;
      return null;
    } catch (err) {
  ...
  const triggerUpdate = async () => {
    ...
    try {
      ...
      if (response.status !== 204) { setStatus("error"); setTimeout(() => setStatus("idle"), 5000); return; }

      await new Promise((resolve) => setTimeout(resolve, 3000));
      const runId = await getLatestRunId();
      if (!runId) { setStatus("error"); setTimeout(() => setStatus("idle"), 5000); return; }
      ...
      pollRef.current = setInterval(async () => {
        ...
        if (count >= 60) { stopPolling(); setStatus("error"); setTimeout(() => setStatus("idle"), 5000); }
      }, 5000);
    } catch (err) {
      ...
      {(status === "triggering" || status === "polling") && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl px-10 py-8 shadow-2xl flex flex-col items-center gap-4 max-w-sm w-full mx-4">
            ...
            <p className="text-white text-xl font-bold text-center">股價更新中</p>
            <p className="text-zinc-400 text-sm text-center">約需 30 秒，請稍候...</p>
            {status === "polling" && elapsedSeconds > 0 && (
              <p className="text-zinc-500 text-xs text-center">已等待 {elapsedSeconds} 秒</p>
            )}
            <div className="w-full bg-zinc-800 rounded-full h-1.5 mt-2">
              <div className="bg-blue-500 h-1.5 rounded-full transition-all duration-1000" style={{ width: `${Math.min((elapsedSeconds / 40) * 100, 95)}%` }} />
            </div>
          </div>
        </div>
      )}
      ...
      {status === "success" && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-zinc-900 border border-green-700 rounded-2xl px-10 py-8 shadow-2xl flex flex-col items-center gap-4 max-w-sm w-full mx-4">
            ...
            <p className="text-white text-xl font-bold text-center">股價更新完成！</p>
            <p className="text-zinc-400 text-sm text-center">畫面數字已更新為最新收盤價</p>
          </div>
        </div>
      )}
      ...
      {status === "error" && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-zinc-900 border border-red-700 rounded-2xl px-10 py-8 shadow-2xl flex flex-col items-center gap-4 max-w-sm w-full mx-4">
            ...
            <p className="text-white text-xl font-bold text-center">更新失敗</p>
            <p className="text-zinc-400 text-sm text-center">請稍後再試，或至 GitHub Actions 手動觸發</p>
          </div>
        </div>
      )}
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="w-full max-w-sm bg-zinc-900 rounded-2xl p-6 shadow-xl border border-zinc-800">
            ...
            <p className="text-zinc-400 text-sm mb-2">現在是下午 2:30 以前，台股尚未收盤。</p>
            <p className="text-yellow-400 text-xs mb-6">
              ⚠️ 更新後顯示的將會是<span className="font-bold">前一個工作天的收盤價</span>，不是今日最新價格。
            </p>
            <div className="flex gap-3">
              <button onClick={() => setShowConfirm(false)} className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-white text-sm py-2.5 rounded-lg transition-colors">取消</button>
              <button onClick={triggerUpdate} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm py-2.5 rounded-lg transition-colors font-medium">確認更新</button>
            </div>
          </div>
        </div>
      )}
